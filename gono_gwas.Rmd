...
sig_threshold <- 0.05/(498*length(antibiotics))
...
```{r}
full_pa_tbl <- fread("./gono_harris_gwas/gene_presence_absence.csv", data.table = FALSE)
...
write.csv(top_anno_gwas_hits, file="./gwases/gono_gwas_annotated.csv", quote = FALSE)
```
...
# threshold form running count_patterns in pyseer
sig_threshold <- 0.05/(1696*length(antibiotics))
